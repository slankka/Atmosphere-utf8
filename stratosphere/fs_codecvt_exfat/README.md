...
This directory contains the ExFAT profile retained from `1.11.2-local`. The
integrated branch builds the FAT32 profile from `stratosphere/fs_codecvt` as the
default. Fusee accepts exactly one FS overlay, so only one profile may be copied
to `sdmc:/atmosphere/fs_overlays/fs_codecvt_unpacked.kip` at a time.
...
`sdmc:/atmosphere/fs_overlays/`。Fusee 当前只接受一个 FS Overlay，因此 FAT32
和 ExFAT 配置不能同时放入该目录。`emummc=0` 只表示进入真实系统，
不会禁用 fs_codecvt。
...
`make` 同时生成压缩中间产物 `fs_codecvt_exfat.kip` 和用于 fusee
overlay 注入的 `fs_codecvt_exfat_unpacked.kip`。部署时必须将后者重命名为：
